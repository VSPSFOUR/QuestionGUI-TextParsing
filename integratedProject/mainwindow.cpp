...
void getLines(string path){
    ...
    // ifstream inputFile("C:/Users/SIDEEQ/Desktop/QT Files/WorldWar2Questions.txt");
    ...
    , ui(new Ui::MainWindow)
{
    ...
    QString progressBarStyleSheet = "QProgressBar {"
                         ...
                         "}"
        ;
    getLines("C:/Users/PC/Documents/QuestionGUI-TextParsing-Grading-Implementation/QuestionGUI-TextParsing-integration/QuestionGUI-TextParsing-integration/integratedProject/WorldWar2Questions.txt");
...
void MainWindow::on_nextButton_clicked()
{
    ...
    // Reset the user's answer for the new question
    Question currentQuestion = testGroup.getQuestion();
    currentQuestion.setUserAnswer(-1); // Reset to 'no answer selected'
    testGroup.setQuestion(currentQuestion);
...
{
    int totalQuestions = testGroup.size();
    int correctAnswers = testGroup.getNumberOfCorrectAnswers();

    // Calculate the percentage score
    double scorePercentage = (static_cast<double>(correctAnswers) / totalQuestions) * 100;

    QString message = "Your final score is " + QString::number(scorePercentage, 'f', 2) + "%.\n";
    message += "You answered " + QString::number(correctAnswers) + " out of " + QString::number(totalQuestions) + " questions correctly.";

    //QString stylesheet = R"(
   // QMessageBox {
     //   background: none; /* Transparent background */
       // color: white; /* White text color */
        //border: 2px solid gray; /* Gray border for visibility */
        //padding: 10px; /* Padding for spacing */
    //}

    //QMessageBox QLabel {
      //  color: inherit; /* Inherit text color from QMessageBox */
    //}
    ...
    msgBox.setWindowTitle("Quiz Results");
...
void MainWindow::on_gradeButton_clicked()
{
    calculateScore();
}


void MainWindow::on_changeIndexButton_clicked()
{
    int newIndex = ui->indexInput->text().toInt();
    testGroup.setIndex(newIndex);
    assignQuestionData(testGroup.getQuestion());
}
